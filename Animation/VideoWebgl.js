// import enableInlineVideo from 'iphone-inline-video' // 加了这个后动画会很卡
import EventEmitter from 'eventemitter3'

export default class VideoWebgl {
  constructor (option) {
    const defaultOption = {
      src: '',
      autoplay: false,
  ...
  initVideo () {
    ...
    if (!video) {
      ...
      video.addEventListener('canplay', () => {
        this.playing = true
        onPlay && onPlay()
        this.event.emit('on-play')
      })
      video.addEventListener('error', () => {
        onError && onError()
        this.event.emit('on-error')
      })
      video.addEventListener('ended', (event) => {
        onPlayEnd && onPlayEnd(event)
        this.event.emit('on-end')
      })
